...
async def handle_query(client, query):
    if query.data == "again":
     ...
     if up:
      ...
      await query.message.reply_photo(up,caption="**Power By @mrdevil12*",reply_markup=markup)
     else:
...
def get_waifu(client, message):
    ...
    if up:
        ...
        message.reply_photo(up,caption="**Power By @mrdevil12**",reply_markup=markup)
    else:
